...
<head>
    ...
    <title>Admin Dashboard - NH Beauty Salon</title>
...
<aside id="sidebar">
    ...
    <div class="sidebar-header">
        <a href="{{ route('admin.dashboard') }}" class="text-decoration-none d-flex align-items-center">
            <img src="{{ asset('assets/US.png') }}" alt="Logo" style="height: 38px;" class="me-2">
            <div class="d-flex flex-column" style="line-height: 1.1;">
                <span class="fw-bold text-dark" style="font-size: 1.1rem; letter-spacing: 1px;">NH BEAUTY</span>
                <span class="fw-medium text-uppercase text-purple-600" style="font-size: 0.55rem; letter-spacing: 3px; color: #581c87;">Salon Admin</span>
            </div>
        </a>
    </div>

    <div class="sidebar-content">
        <a href="{{ route('admin.dashboard') }}" class="nav-link-kasir {{ Request::is('admin/dashboard') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.kategori.index') }}" class="nav-link-kasir {{ Request::is('admin/kategori*') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.laporan.keluar') }}" class="nav-link-kasir {{ Request::is('admin/laporan/keluar*') ? 'active' : '' }}">
            <i class="bi bi-graph-down-arrow"></i>
            <span>Pengeluaran</span>
        </a>

        <div class="px-4 py-2 mt-3 mb-1 small text-uppercase text-muted fw-bold" style="letter-spacing: 1px; font-size: 0.7rem;">Kelola Salon</div>

        <a href="{{ route('admin.salon.profile') }}" class="nav-link-kasir {{ Request::is('admin/salon/profile*') ? 'active' : '' }}">
            <i class="bi bi-shop"></i>
            <span>Profil Salon</span>
        </a>
        <a href="{{ route('admin.salon.gallery') }}" class="nav-link-kasir {{ Request::is('admin/salon/gallery*') ? 'active' : '' }}">
            <i class="bi bi-info-circle"></i>
            <span>Informasi Galeri</span>
        </a>

        <div class="px-4 py-2 mt-3 mb-1 small text-uppercase text-muted fw-bold" style="letter-spacing: 1px; font-size: 0.7rem;">Sistem</div>

        <a href="{{ route('admin.pengguna.index') }}" class="nav-link-kasir {{ Request::is('admin/pengguna*') ? 'active' : '' }}">
    ...
    <div class="p-3">
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-outline-danger w-100 rounded-4 py-2 d-flex align-items-center justify-content-center gap-2">
...
<header id="topbar">
    <div class="d-flex align-items-center gap-3">
        ...
        <h5 class="mb-0 fw-bold d-none d-md-block topbar-title">Admin Panel</h5>
    </div>
    ...
    <div class="dropdown">
        <div class="d-flex align-items-center gap-3 cursor-pointer" data-bs-toggle="dropdown">
            ...
            <div class="rounded-circle overflow-hidden bg-white d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                @if(Auth::user()->foto_p)
